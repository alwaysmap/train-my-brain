...
description: >
  ...
  Triggers on "/tmb:review", "re-review this curriculum", "check for drift",
...
- If flags exist: tell the user how to approve them — open `review.md`, set `approved: true` on flags they want applied, then re-run `/tmb:review`.
...
If the user wants a post-review build refresh, point them at `/tmb:rebuild-site` (or `./build.sh` from the curriculum root).
